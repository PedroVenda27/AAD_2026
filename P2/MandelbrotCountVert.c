...
//
// Mandelbrot set count, parallel version 1: static division in vertical stripes
// each thread processes a contiguous range of re_idx (a "column stripe"), all im_idx
//
...
{
  int thread_number;
  int re_start,re_end; // half-open range [re_start,re_end)
  long count;
}
thread_data_t;
...
static void *worker_thread(void *arg)
{
  ...
  long local_count = 0;
  for(int re_idx = td->re_start;re_idx < td->re_end;re_idx++)
  {
...
int main(int argc,char **argv)
{
  ...
  double t1 = wall_time();
  printf("%ld -- %7.3f (n_threads=%d, vertical stripes)\n",total,t1 - t0,n_threads);
  return 0;
}
